...
namespace AtharERP_System.Controllers
{
    ...
    public class ProjectAssignmentsController : Controller
    {
        ...
        public async Task<IActionResult> Overview(int? projectId, int? stageId, string? taskFilter, string? docFilter)
        {
            ...
            ViewBag.DocFilter = string.IsNullOrEmpty(docFilter) ? "all" : docFilter;
...
List<int>? taskIds)
        ...
        public async Task<IActionResult> EditDates(int id, int? taskId, [Bind("AssignmentType,PlannedStartDate,PlannedEndDate")] ProjectAssignment model)
        {
            ...
            return taskId.HasValue
                ? this.RedirectKeepingTab("Edit", "ProjectTasks", new { id = taskId.Value })
                : this.RedirectKeepingTab("ManageTasks", new { id = assignment.Id });
        }
        ...
        // ============================================
        // شاشة إدارة بنود التكليف الكاملة (تفتح عند نقر المهندس على أي تكليف من "تكليفاتي")
        // ============================================
        [Authorize]
        [HttpGet]
