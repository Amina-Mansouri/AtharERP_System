...
## ١. طبقة العرض موجودة — والمطلوب إعادة تصميمها لا إنشاؤها
تصحيح: الـ Views موجودة ومختبَرة **محلياً** عبر الوحدات ٠١/٠٢/٠٣، لكنها **غير مرفوعة
إلى GitHub** (المتتبَّع فقط `Views/Shared/_Layout.cshtml.css`) — فأي قراءة عن بُعد
للمستودع تبدو كأنها غير موجودة. **المصدر الصحيح هو النسخة المحلية.**

لذلك:
- عدّل `_Layout.cshtml` القائم ولا تستبدله: استبدل الـ Navbar بشريط علوي + قائمة جانبية،
  و**احتفظ بكل شروط الصلاحيات وأسماء الـ actions والروابط الموجودة كما هي**.
- أعد تصميم كل View قائم مكان إنشاء view جديد بنفس الاسم.
- لا تغيّر أسماء الحقول أو الـ actions أو الصلاحيات لتطابق المواصفة — المواصفة تصف
  **الشكل**؛ الأسماء الحقيقية في الكود هي المرجع.
- **الشاشات الشاملة الناقصة فعلاً**: المهام، التكاليف، الجدول الزمني (عبر كل المشاريع)،
  وقوائم الوحدة ٠٣ الشاملة (التقارير اليومية، الفحوصات، المقاولون، الصيانة، طلبات التوريد)
  — الموجود نُسخ مرتبطة بمشروع/موقع محدَّد. تُبنى في المرحلتين ٤ و٥؛ ومؤقتاً يُربط بند
  القائمة بأقرب صفحة فعلية مع `// TODO` واضح.
- يُنصح برفع الـ Views إلى المستودع (`git add Views/`) حتى تُقرأ صحيحة لاحقاً.
